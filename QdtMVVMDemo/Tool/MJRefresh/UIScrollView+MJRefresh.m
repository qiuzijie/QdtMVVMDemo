...
static char MJRefreshHeaderCommandKey;
...
- (void)setHeaderBeginRefreshingCommand:(RACCommand *)headerBeginRefreshingCommand{
    [self willChangeValueForKey:@"MJRefreshHeaderCommandKey"];
    objc_setAssociatedObject(self, &MJRefreshHeaderCommandKey,
                             headerBeginRefreshingCommand,
                             OBJC_ASSOCIATION_ASSIGN);
    [self didChangeValueForKey:@"MJRefreshHeaderCommandKey"];
}

- (RACCommand *)headerBeginRefreshingCommand{
    return objc_getAssociatedObject(self, &MJRefreshHeaderCommandKey);
}
...
- (void)addHeaderWithCommand:(RACCommand *)command{
    // 1.创建新的header
    if (!self.header) {
        MJRefreshHeaderView *header = [MJRefreshHeaderView header];
        [self addSubview:header];
    ...
    // 2.设置block回调
    if (command) {
        self.header.beginRefreshingCommand = command;
    }
}
...
- (void)addFooterWithCommand:(RACCommand *)command{
    // 1.创建新的footer
    if (!self.footer) {
        MJRefreshFooterView *footer = [MJRefreshFooterView footer];
        [self addSubview:footer];
    ...
    self.footer.beginRefreshingCommand = command;
}
